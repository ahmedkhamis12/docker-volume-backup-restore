...
# # Run the backup using Alpine with zip installed
# docker run --rm \
#   -v "$VOLUME_NAME":/data \
...
# # --- Prepare remote EC2 backup folder ---
# echo "🛠️ Preparing backup directory on EC2..."
# ssh -i "$EC2_KEY_PATH" -o StrictHostKeyChecking=no "$EC2_USER@$EC2_IP" \
#   "mkdir -p $EC2_BACKUP_PATH && chmod 755 $EC2_BACKUP_PATH"

# # --- Upload to EC2 using SCP ---
# echo "📤 Uploading to EC2 server..."
# # scp -i "$EC2_KEY_PATH" -o StrictHostKeyChecking=no \
# #   "$BACKUP_DIR/$BACKUP_FILE" \
# #   "$EC2_USER@$EC2_IP:$EC2_BACKUP_PATH"
# scp -i "$SSH_KEY_PATH" "$REMOTE_USER@$REMOTE_HOST:$REMOTE_BACKUP_PATH/$LATEST_BACKUP" "$LOCAL_BACKUP_DIR/"
...
# REMOTE_HASH=$(ssh -i "$EC2_KEY_PATH" "$EC2_USER@$EC2_IP" "sha256sum '$EC2_BACKUP_PATH/$BACKUP_FILE' | awk '{print \$1}'")
